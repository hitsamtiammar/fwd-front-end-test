...
export default function TicTacToe({ numCol, onExit }) {
  ...
  function checkDiagonal() {
    const funcs = [checkLeftDiagonal, checkRightDiagonal];
  ...
  function renderBackdrop() {
    return (
      <div
        ...
        } flex-column justify-content-center align-items-center backdrop item-3`}
      >
        <h1 className="mb-3 fw-bold">
          {winner ? `${PLAYERS[winner]} win !` : 'Draw!'}
        </h1>
        <div className="d-flex flex-row">
          <Button onClick={onRetryClicked} variant="primary" className="me-2">
            Retry
          </Button>
  ...
  return (
    <>
      <div className="d-flex flex-row align-items-center me-3">
        <strong className={`fs-2 ${!turn && 'text-success'}`}>
          O: {countWinner.o}{' '}
        </strong>
      </div>
      <div className="d-flex flex-column main-container">
        ...
        {rows.map((item, rowIndex) => (
          <div
            key={`${item.length}-${rowIndex}`}
            className="d-flex flex-row i-row w-100"
          >
            {item.map((item2, colIndex) => (
              <div
                key={`${item.length}-${item2}-${colIndex}`}
                onClick={() => onItemClick(rowIndex, colIndex)}
                className={`item item-${numCol} border border-secondary`}
              >
                {item2 !== '' ? (
                  <Lottie
                    style={{
                    ...
                    animationData={getAnimationData(item2)}
                    loop={false}
                  />
                ) : null}
              </div>
